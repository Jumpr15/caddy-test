...
func main() {
	ue := UpstreamEndpoint{Id: "upstream-worker-1", Address: "127.0.0.1", Port: "80"}
	c := CaddyClient{Address: "127.0.0.1", Port: "2019"}
	// c.GetUpstreamDetails("upstream-worker-1")
	c.GetUpstreamDetails(ue)
	c.AddUpstreamEndpoint(ue)
	c.GetUpstreamDetails(ue)
}